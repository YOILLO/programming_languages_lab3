...
static struct region alloc_region  ( void const * addr, size_t query ) {
	...
	new_reg.addr = new_address;
	new_reg.size = query;
	block_size size;
	size.bytes = new_reg.size;
	block_init(new_address, size, NULL);
	return new_reg;
}
...
static bool split_if_too_big( struct block_header* block, size_t query ) {
	if (block_splittable(block, query)) {
		...
		next_block_size.bytes = block->capacity.bytes - query;
		block->capacity.bytes = query;
		void* next_block = block_after(block);
		block_init(next_block, next_block_size, block->next);
		block->next = (struct block_header*) next_block;
		return true;
	} else
		return false;
}
...
static struct block_search_result find_good_or_last  ( struct block_header* restrict block, size_t sz )    {
	...
	while (block_current) {
		while (block_current->is_free && (block_is_big_enough(sz, block_current) || try_merge_with_next(block_current))) {
			if (block_is_big_enough(sz, block_current)) {
				struct block_search_result result;
				result.type = BSR_FOUND_GOOD_BLOCK;
				result.block = block_current;
				return result;
			}
		}
...
static struct block_header* grow_heap( struct block_header* restrict last, size_t query ) {
	...
	if (region_is_invalid(&new_region)) {
		new_region = alloc_region(NULL, size.bytes);
		if (region_is_invalid(&new_region))
			return NULL;
	}	
